...
corriger pour quer Form.eleCouQuoiPos soit à zéro \
2. normalement, on aura (Form.eleCouQuoiPos=0) \
...
If ((Form:C1466.quoiListe.length<2)\
 | (Form:C1466.eleCouQuoiPos=0))
...
Case of 
	...
	: (Form:C1466.action="PRECEDENT")
		If (Form:C1466.eleCouQuoiPos>1)
			Form:C1466.eleCouQuoiPos:=Form:C1466.eleCouQuoiPos-1
		Else 
			Form:C1466.eleCouQuoiPos:=Form:C1466.quoiListe.length
		End if 
		
	: (Form:C1466.action="SUIVANT")
		If (Form:C1466.eleCouQuoiPos<Form:C1466.quoiListe.length)
			Form:C1466.eleCouQuoiPos:=Form:C1466.eleCouQuoiPos+1
		Else 
			Form:C1466.eleCouQuoiPos:=1
		End if 
...
// après mise à jour du "Form.eleCouQuoiPos", blanchir "Form.action" (car utilisé dans déplacement pg2 et pg3)
Form:C1466.action:="MODIFIER"

//"Form.quoiListe" est vide: aucun quoi trouvé suivant les critères, \
du coup l'utilisateur en crée un mais la liste reste vide.... CQFD 
If ((Form:C1466.quoiListe.length=0)\
 | (Form:C1466.eleCouQuoiPos=0))
	// pour test: CONFIRMER("charger_unQuoi: Form.quoiListe.length=0")
Else 
	//les info sur le quoi sélectionné: pour affichage détail
	C_LONGINT:C283($ind)
	$ind:=Form:C1466.eleCouQuoiPos-1  //si position=1, alors indice=0
	Form:C1466.quoi:=Form:C1466.quoiListe[$ind]
